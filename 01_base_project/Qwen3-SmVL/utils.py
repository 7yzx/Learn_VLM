...
DEFAULT_VISION_MODEL_PATH = "model/SmolVLM2-256M-Video-Instruct"
DEFAULT_TEXT_MODEL_PATH = "model/Qwen3-4B"


def load_processor(
    text_model_name_or_path: str = DEFAULT_TEXT_MODEL_PATH,
    vision_model_name_or_path: str = DEFAULT_VISION_MODEL_PATH,
):
    """
    加载和配置数据处理器
    ...
    print("正在加载Qwen3分词器...")
    qwen3_tokenizer = AutoTokenizer.from_pretrained(text_model_name_or_path)
    if qwen3_tokenizer.pad_token_id is None:
    ...
    if image_token_id is None or image_token_id < 0:
        raise ValueError("Qwen3 tokenizer 中缺少 <|image_pad|> special token")
    
    # 配置特殊token
    smolvlm2_processor.fake_image_token = "<vision_start>"
    smolvlm2_processor.image_token = "<|image_pad|>"
    smolvlm2_processor.image_token_id = image_token_id
...
def load_model(
    ...
    text_hidden_size = qwen3_model.config.hidden_size
    ...
    print("正在替换语言模型组件...")
    smolvlm2_02B_model.model.text_model = qwen3_model.model
    smolvlm2_02B_model.lm_head = qwen3_model.lm_head
    ...
    smolvlm2_02B_model.config.text_config.hidden_size = text_hidden_size
    smolvlm2_02B_model.model.config.text_config.hidden_size = text_hidden_size
    ...
    smolvlm2_02B_model.generation_config.eos_token_id = qwen3_tokenizer.eos_token_id
    smolvlm2_02B_model.generation_config.pad_token_id = qwen3_tokenizer.pad_token_id
