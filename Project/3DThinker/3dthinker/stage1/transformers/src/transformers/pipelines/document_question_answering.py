...
class DocumentQuestionAnsweringPipeline(ChunkPipeline):
    ...
    def postprocess_extractive_qa(
        ...
        for output in model_outputs:
            ...
            if self.framework == "pt" and output["start_logits"].dtype in (torch.float16, torch.float16):
                output["start_logits"] = output["start_logits"].float()
            if self.framework == "pt" and output["end_logits"].dtype in (torch.float16, torch.float16):
