...
Code Llama のベースとなる`Llama2`ファミリー モデルは、`float16`を使用してトレーニングされましたが、元の推論では`float16`を使用します。さまざまな精度を見てみましょう。
...
* `float16`: コード Llama はこの精度でトレーニングされているため、さらなるトレーニングや微調整に使用することをお勧めします。
* `float16`: この精度を使用して推論を実行することをお勧めします。通常は `float16` より高速であり、評価メトリクスには `float16` と比べて明らかな低下が見られないためです。 float16 を使用して推論を実行することもできます。微調整後、float16 と float16 の両方で推論結果を確認することをお勧めします。
