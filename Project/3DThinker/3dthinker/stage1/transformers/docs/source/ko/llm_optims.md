...
Mistral-7B-v0.1을 반정밀도로 로드하려면 [`~transformers.AutoModelForCausalLM.from_pretrained`] 메서드에서 `torch_dtype` 매개변수를 `torch.float16`으로 설정하십시오. 이 경우 13.74GB의 메모리가 필요합니다.
