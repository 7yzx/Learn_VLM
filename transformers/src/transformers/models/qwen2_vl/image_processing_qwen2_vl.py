...
"""Vision数据Patch处理代码 Image processor class for Qwen2-VL."""
...
class Qwen2VLImageProcessor(BaseImageProcessor):
    ...
    def _preprocess(
        ...
        patches = patches.reshape(
            grid_t,              # 时间块数 (视频帧 / 2)
            temporal_patch_size, # 每组时间块包含的帧数
            channel,             # 通道数
            grid_h // merge_size,# 高度方向的块数
            merge_size,          # 每组高度块包含的patch数
            patch_size,          # 每个小patch的高度像素
            grid_w // merge_size,# 宽度方向的块数
            merge_size,          # 每组宽度块包含的patch数
            patch_size,          # 每个小patch的宽度像素
        )
